    ...
    for root, _, files in os.walk(processed_dir):
        for file in files:
            if file.lower().endswith('.mp4'):
                ...
                # Определяем метку в зависимости от директории
                if os.path.basename(root) == "correct":
                    label = 1
                elif "incorrect" in relative_path:
                    label = 0
                else:
                    continue  # Пропускаем файлы из других папок

                poses_subdir = os.path.join(poses_dir, relative_path)
                os.makedirs(poses_subdir, exist_ok=True)

                print(f"Обработка видео: {video_path}")
                landmarks = pose_analyzer(video_path, label)
